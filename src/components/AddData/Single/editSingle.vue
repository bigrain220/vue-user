<template>
	<div class="editSingle">
		...
		<el-form :model="Editform" :rules="rules" label-width="100px" ref="Editform" id="editform">
			<el-collapse v-model="activeName">
				...
				<el-collapse-item title="添加内容" name="2">
					...
					<el-form-item label="路径名" prop="url_name">
						<el-input v-if="Editform.id > 4" v-model="Editform.url_name"></el-input>
						<el-input v-else disabled v-model="Editform.url_name"></el-input>
					</el-form-item>
					<el-form-item label="模板名" prop="tpl_name">
						<el-input v-model="Editform.tpl_name" placeholder="默认模板名为 index"></el-input>
					</el-form-item>
					<el-form-item label="排序" prop="level" class="level">
						<el-input-number v-model="Editform.level" :min="0" controls-position="right"></el-input-number>
					</el-form-item>
			...
			<el-form-item class="last-item">
				<el-button type="primary" @click="submitForm('Editform')">修改</el-button>
					<router-link to="/user/Single"><el-button>返回</el-button></router-link>
			</el-form-item>
		</el-form>
	</div>
</template>
<script>
	...
	export default {
		...
		data() {
			...
			return {
				...
				rules:{
					...
					url_name:[{ required: true, validator: checkName, trigger: 'blur' }],
		...
		methods: {
			...
			getEditSingle(id){
				API.SingleList({id:id}).then(rs => {
					this.Editform = rs.rows[0];
					this.Editform.level = Number(this.Editform.level);
					// this.image = this.Editform.image;
				}).catch(() => {
					console.log('get EditSingle error!');
				});
			},
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if(valid){
						API.SingleEdit(this.Editform).then(rs => {
							if(rs.code == 1){
		...
		mounted:function(){
			this.getEditSingle(this.$route.params.editId);
